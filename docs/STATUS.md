...
- **G19 docs 허브** — 별도 세션이 상세 문서 17개를 채움(하드코딩 page.tsx). **⚠️ 전부 한국어 전용(EN 라우트에 KO 콘텐츠, ko/jp 상세 라우트 없음)** → **임시 조치(2026-07-15)**: EN/JP 방문자에 "한국어 문서, 번역 준비 중" 클라이언트 안내 배너(`DocTranslationNotice` + `src/app/resources/docs/layout.tsx`, rewrite가 브라우저 URL 보존→pathname으로 로케일 감지, KO 콘텐츠·17페이지 무변경=동시세션 충돌 없음). ko/jp 상세는 proxy rewrite로 한국어 서빙(200 검증). **잔여=정식 EN/JP 번역**(대규모, 이후 — 로케일 인식 리팩터/컬렉션화 검토).
