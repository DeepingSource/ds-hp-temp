> 📌 **읽기 전 — 이 INDEX 의 현재 자리 (2026-06-19 정리)**
> 본 문서가 카탈로그로 묶는 **Phase 1–4 계획 산출물(2026-05-29)**은 history 보존용이다. **사이트는 이 계획을 지나 이미 출하**됐다 — 라이브 SOT 는 repo-root **[`/DESIGN.md`](../DESIGN.md)** + 라이브 **`src/`** 다.
> 본문이 `22→30` 라우트를 전제하지만 **현재 라우트는 `src/app` 기준 약 115 page.tsx** 다. 아래 §1 표에서 **SOT** 로 표기된 `_v1`/`_v2` 산출물(PLAN_v1.1·BRAND_v2·CASE_STUDIES_v1·DESIGN_v2·CODE_v1 등)은 이제 모두 **Archive** 로 읽어야 한다.
