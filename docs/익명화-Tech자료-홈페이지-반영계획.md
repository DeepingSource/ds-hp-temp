...
>
> **2차 — 실자산 반영 (법무 게이트 해소 후).** 보유 영상·이미지가 **stock 구매·처리본**이라 L1/L2 권리 해소됨(사용자 확인). 웹 처리(ffmpeg 8.1/cwebp) 후 AnonymizerView에 실자료 2종 투입: (1) **before↔after 드래그 슬라이더** — `face-before/after.png`→webp(4.5/11KB), 재사용 컴포넌트 `ui/BeforeAfterSlider.tsx`(pointer/range 접근성), Mechanism 파이프라인 아래. 실제 노이즈 출력 노출. (2) **Pete 킬러 데모** — `pete_face_anonymized2.mov`→`pete-anon-demo.mp4`(1.34MB)+`.webm`(1.47MB)+poster.webp, 재사용 컴포넌트 `ui/LoopVideo.tsx`(IntersectionObserver lazy + reduced-motion), "얼굴은 지워도 맥락은 읽습니다" 밴드(Mechanism↔Limitation). 카피 Part 1 §2/§3, KO/EN/JP. **나머지 보류** — 3분할 `anonymizer_demo`·SealView full/partial 비교·home TrustCharter 슬라이더·spatial-ai 등은 다음 증분.
>
> **3차 — 기술 영역 전반 실자산 반영 완료 (법무 게이트 해소).** 남은 영상 변환(노이즈=고엔트로피라 webm이 mp4보다 커서 **mp4 단일**: 3분할 2.35MB·full 767KB·partial 967KB) 후 5개 뷰 와이어링: (1) **홈 TrustCharter** 정지 dissolve→`BeforeAfterSlider` 교체, (2) **SpatialAiView** Pete 데모 밴드(LoopVideo), (3) **ModelsView** face-anon 실 before/after 슬라이더 섹션(합성 카탈로그 위), (4) **TechnologyView** 합성 AnonymizationMockup→`anon-3panel-demo` 풀폭 실영상(원본→익명화→추적) + honesty 캡션, (5) **SealView** full vs partial 2-up 비교 영상. 전부 KO/EN/JP(plan Part 1), poster·lazy·reduced-motion·aria. 검증: build green, lint 클린, Playwright 5곳 렌더 확인. Pete 속성 말풍선은 여전히 영어(A3) — 로케일 캡션으로 보완.
>
> **4차 — iStock 얼굴 검출 반영 완료.** `face-landmark-demo.mp4`(646KB)+webm+poster, SpatialAiView "비전 모델 · 얼굴 검출" 밴드. ⚠️ 익명화-우선 브랜드에 실제 얼굴+랜드마크 메시는 자칫 얼굴인식으로 읽힐 위험 → AnonymizerView mechanism 논리("신원 영역 찾기 = 알아내기 위해서가 아니라 지우기 위해")를 빌려 **"찾아내는 건, 지우기 위해서입니다"** 프레이밍 + "찾은 자리는 곧 익명화됩니다" 캡션으로 정합. **기술 영역 실자산 반영 전부 완료.**
