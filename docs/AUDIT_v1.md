> 📌 **읽기 전 — 이 감사의 자리 (2026-06-19 정리)**
> 본 문서는 **동결된 Phase 1–4 계획** 대비 결함(F-01..F-27)을 추적하며, **후속(AUDIT_v2)이 없다**. 사이트는 이 계획을 지나 이미 출하됐으므로, 여기 stale 한 F-ID 들을 일일이 대조·해소하기보다 **현행 `src/` 기준으로 새 감사를 돌리는 편이 더 유용**하다 — 본 문서는 history 로 보존한다.
