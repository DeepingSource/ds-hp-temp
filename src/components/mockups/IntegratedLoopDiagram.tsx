...
const COPY: Record<Locale, Copy> = {
  ko: {
    ...
    svgDesc:
      '입력(CCTV·POS·외부 컨텍스트)이 SEAL과 Anonymizer를 거쳐 익명화 이벤트 허브로 모이고, Insight·Care·Agent 세 갈래로 흐른 뒤 사람의 결정과 행동으로 이어지며, 그 결과가 다시 허브로 되먹임되는 운영 루프.',
  ...
  en: {
    ...
    lead: 'Insight makes sense of yesterday. Care tells you what is happening now. Agent points to what comes next. The operator’s actions feed back and make all three sharper.',
    nodes: {
    ...
    svgDesc:
      'Inputs (CCTV, POS, external context) pass through SEAL and the Anonymizer into an anonymized event hub, branch into Insight, Care and Agent, lead to human decision and action, and feed the outcome back into the hub.',
    scrollLabel: 'Integrated signal-flow diagram — scroll horizontally to explore',
  },
  jp: {
    eyebrow: '三つが揃うと',
    heading: '三つの時間軸が一つの運用ループへと閉じます',
    lead: 'Insight が昨日を整理します。Care が今を知らせます。Agent が次を示します。運用者の行動の結果がフィードバックとなり、三つすべてをより賢くします。',
    nodes: {
    ...
    svgDesc:
      '入力（CCTV・POS・外部コンテキスト）が SEAL と Anonymizer を経て匿名化イベントハブに集まり、Insight・Care・Agent の三方向へ流れ、人の判断・行動へとつながり、その結果が再びハブへフィードバックされる運用ループです。',
...
const HUB_W = 168;
const ROW = [80, 150, 220] as const; // triple-stack y-centers
const MID = 150;
      ...
      >
        ...
        >
          ...
          <g className="stroke-gray-400" fill="none" strokeWidth={1.6} markerEnd="url(#ild-ah)">
            <path d="M 156 80 C 185 80, 195 138, 211 138" />
            <path d="M 156 150 L 211 150" />
            <path d="M 156 220 C 185 220, 195 162, 211 162" />
          ...
          <SvgNode x={COL.in} cy={ROW[0]} label={t.nodes.cctv.label} tip={t.nodes.cctv.tip} active={activeStage === 0} />
          <SvgNode x={COL.in} cy={ROW[1]} label={t.nodes.pos.label} tip={t.nodes.pos.tip} active={activeStage === 0} />
          <SvgNode x={COL.in} cy={ROW[2]} label={t.nodes.ext.label} tip={t.nodes.ext.tip} active={activeStage === 0} />
